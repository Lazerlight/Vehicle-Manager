...
namespace WinFormsApp1
{
    public partial class CarViewerView : Form
    {
            ...
            using (SqlConnection con = new SqlConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        if (ID == id && monthProp == M && yearProp == Y)
                        {
        ...
        public string monthProp
        {
            get { return dateValueM.Text; }
            set { dateValueM.Text = value; }
        }
        public string yearProp
        {
            get { return dateValueY.Text; }
            set { dateValueY.Text = value; }
        }
        ...
        private void editButton_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection())
            {
                ...
                string M = dateValueM.Text.Substring(0, dateValueM.Text.IndexOf('.'));
                string Y = dateValueY.Text.Substring(0, dateValueY.Text.IndexOf('г'));
                con.ConnectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\VehicleManagerApplication\SQLlocalDBforVM.mdf;Integrated Security=True;Connect Timeout=30";
                con.Open();
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.CommandText = $"update vehicle set Id=N'" + carNumberValue.Text + "'," +
                        ...
                        " OMW=N'" + omwValue.Text + "'," +
                        " LUKOIL=N'" + lukoilValue.Text + "' where (Id= N'" + carNumberValue.Text + "'AND Month='" + M + "' AND Year='" + Y + "')";
                    cmd.ExecuteNonQuery();
